...
if st.session_state.get("show_cart", False):
    ...
    if len(st.session_state.cart) == 0:
        ...
        buyer_address = st.text_area("Delivery Address *")
        st.markdown('</div>', unsafe_allow_html=True)
        st.write("### Your Order")
        for item in st.session_state.cart:
            st.write(f"- {item['name']} - ₦{item['price']:,}")
        st.write(f"### Total: ₦{total:,}")
        order_details = f"New Order from VEYRA%0A%0AName: {buyer_name}%0APhone: {buyer_phone}%0AEmail: {buyer_email}%0AAddress: {buyer_address}%0A%0AORDER:%0A" + "%0A".join([f"- {p['name']} - ₦{p['price']:,}" for p in st.session_state.cart]) + f"%0A%0ATotal: ₦{total:,}"
        whatsapp_link = f"https://wa.me/{OWNER_WHATSAPP}?text={order_details}"
        if buyer_name and buyer_phone and buyer_email and buyer_address:
