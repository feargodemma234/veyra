...
OWNER_EMAIL = "veyraofficial@gmail.com"
...
if st.session_state.get("show_cart", False):
    ...
    if len(st.session_state.cart) == 0:
        ...
        if buyer_name and buyer_phone and buyer_email and buyer_address and buyer_state:
            st.markdown(f'<div class="buy-btn">', unsafe_allow_html=True)
            st.markdown(f'<a href="{whatsapp_link}" target="_blank"><button>Send Order via WhatsApp</button></a>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
            st.markdown('<div class="email-btn">', unsafe_allow_html=True)
            if st.button("Send Order + ID to Email"):
                data = {
                ...
                if send_to_formspree(data, files):
                    st.success("Order received! Check your email.")
                else:
                    st.error("Failed to send. Check Formspree setup.")
            st.markdown('</div>', unsafe_allow_html=True)
        else:
            st.warning("Please fill all your details to continue")
